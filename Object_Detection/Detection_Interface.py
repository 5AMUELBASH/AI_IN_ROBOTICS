...
model_path = 'Object_Detection/runs/detect/train/weights/best.pt'
...
class StatisticsCollector:
    """Collects and analyzes detection statistics"""
    ...
    def add_detection(self, detections, processing_time):
    ...
    def _create_summary_page(self, pdf):
        ...
        # Title
        ax.text(0.5, 0.95, 'Detection Analysis Report', 
        ...
        info_text = (
            ...
            f"Detection Rate: {total_detections/max(1, self.frame_count):.2f} detections/frame\n"
        )
        ...
        for class_name, count in sorted(self.class_counts.items(), key=lambda x: x[1], reverse=True):
            if count > 0:
                class_summary += f"  {class_name}: {count} ({count/max(1, total_detections)*100:.1f}%)\n"
    ...
    def _create_temporal_analysis(self, pdf):
        """Create temporal analysis of detections"""
        ...
        ax1.set_title('Detection Frequency Over Time', fontweight='bold')
...
class LiveDetectorGUI:
    ...
    def setup_webcam(self):
        ...
        for _ in range(5):
            ...
            if ret and frame is not None:
                self.parent.update_status("Webcam connected - Starting detection...")
                return True
            time.sleep(0.1)
            ...
            if current_time - self.last_detection_time >= self.detection_interval:
                try:
                    if not self.frame_queue.empty():
                        ...
                        # Add to statistics
                        if self.stats:
                            self.stats.add_detection(detections, self.processing_time)
    ...
    def add_annotations(self, frame):
        """Add performance info to frame (removed keybind instructions)"""
        height, width = frame.shape[:2]
        ...
        # Performance info only (top of frame)
    ...
    def main_loop(self):
        ...
        # Update status
        status_text = f"FPS: {self.fps:.1f} | Objects: {len(self.latest_detections)} | Recording: {'ON' if self.recording else 'OFF'}"
...
class MainApplication:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("YOLOv8 Object Detection - Enhanced")
        self.root.geometry("900x700")  # Slightly larger to accommodate controls
    ...
    def show_main_menu(self):
        ...
        title_label = tk.Label(title_frame, text="YOLOv8 Object Detection", 
                              font=("Arial", 28, "bold"), fg="white", bg="#2c3e50")
        title_label.pack(pady=10)
        
        subtitle_label = tk.Label(title_frame, text="Enhanced Detection System", 
        ...
        analyze_btn = ModernButton(button_frame, "Analyze Images from Folder", 
                                 command=self.show_image_analysis_screen,
                                 width=300, height=60, color="#27ae60", hover_color="#219653")
        analyze_btn.pack(pady=20)
        
        # Live Detection button
        live_btn = ModernButton(button_frame, "Live Webcam Detection", 
                              command=self.show_live_detection_screen,
    ...
    def show_statistics_dashboard(self):
        ...
        stats_text = (
            ...
            f"Total Detections: {total_detections}\n"
            f"Detection Rate: {total_detections/max(1, total_frames):.2f} detections/frame\n"
        )
        ...
        for class_name, count in sorted(self.stats.class_counts.items(), key=lambda x: x[1], reverse=True):
            if count > 0:
                ...
                class_label = tk.Label(class_item, 
                                      text=f"{class_name}: {count} ({count/max(1, total_detections)*100:.1f}%)",
    ...
    def show_image_analysis_screen(self):
        """Display the image analysis screen with enhanced controls"""
        self.clear_screen()
        ...
        # Title
        title_label = tk.Label(main_frame, text="Image Analysis", 
                              font=("Arial", 24, "bold"), fg="white", bg="#34495e")
        title_label.pack(pady=20)
        
        # Compact settings frame
        settings_frame = tk.LabelFrame(main_frame, text="Detection Settings", 
        ...
        self.output_folder_var = tk.StringVar(value=os.path.join(os.getcwd(), "detection_results"))
        ...
        process_btn = ModernButton(button_frame, "Process Images", 
    ...
    def browse_output_folder(self):
        ...
        folder_path = filedialog.askdirectory(title="Select Output Folder for Detection Results")
        if folder_path:
    ...
    def _process_images_thread(self, image_paths, output_folder):
        ...
        try:
            ...
            for i, image_path in enumerate(image_paths):
                ...
                # Draw detections
                annotated_image = self.draw_detections_on_image(image, results[0])
    ...
    def draw_detections_on_image(self, image, result):
        """Draw detections on a single image with current settings"""
        ...
        if result.boxes is not None and len(result.boxes) > 0:
            for box in result.boxes:
                ...
                text_bg_y1 = max(0, y1 - text_height - baseline - 10)
                cv2.rectangle(
                    annotated_image, 
                    (x1, text_bg_y1), 
                    (x1 + text_width + 10, y1), 
    ...
    def show_live_detection_screen(self):
        """Display the live detection screen with enhanced controls"""
        if not self.model:
            messagebox.showerror("Error", "Model not loaded!")
            return
            
        self.clear_screen()
        self.current_screen = "live_detection"
        self.root.configure(bg="#1a1a1a")
        
        # Set a larger window for live detection but not full screen
        ...
        title_label = tk.Label(title_frame, text="Live Object Detection", 
                              font=("Arial", 20, "bold"), fg="white", bg="#1a1a1a")
        title_label.pack()
        
        subtitle_label = tk.Label(title_frame, text="Real-time Detection with Enhanced Controls", 
        ...
        self.video_frame.pack(expand=True)  # Center the video frame but don't let it expand infinitely
        self.video_frame.pack_propagate(False)  # Prevent the frame from resizing to fit content
        ...
        self.live_detector = LiveDetectorGUI(self.model, self, self.stats)
        self.root.after(100, self.start_live_detection)
    
    def update_live_class_filter(self):
        """Update class filter for live detection"""
    ...
    def toggle_recording(self):
        """Toggle recording state"""
        if hasattr(self, 'live_detector'):
            self.live_detector.toggle_recording()
    
    def stop_live_detection(self):
        """Stop live detection and return to main menu"""
        if hasattr(self, 'live_detector'):
            self.live_detector.stop()
            del self.live_detector
    ...
    def update_video_frame(self, frame):
        ...
        if self.current_screen != "live_detection":
            return
        ...
        display_width = 800  # Fixed width
        display_height = 600  # Fixed height
